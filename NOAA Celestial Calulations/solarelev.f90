...
!
    ...
    contains
       function solarelevation(lat,lon,year,month,day,hours,minutes,     &
...
!
      longitude= -lon
      latitude=lat
      if( latitude>89.80D0 )latitude=89.8D0
      ...
      timenow=hh+minutes/60.0D0+seconds/3600.0D0+zone
...
!      end do
      ...
      if( csz<-1.0D0 )csz=-1.0D0
      if(abs(csz) > 1.0d0)csz = 1.0d0*sign(1.0d0,csz)
      temp3=acos(csz)
      zenith=acosd(csz)
      azdenom=cosd(latitude)*sin(temp3)
      if( abs(azdenom)>0.001D0 )then
         azrad=(sind(latitude)*csz-sind(solardec))/azdenom
         if( abs(azrad)>1.0D0 )then
            mysign = sign(1.0d0,azrad)
            azrad = 1.0d0*mysign
!            if( azrad<0.0D0 )then
!               azrad=-1.0D0
!            else
...
!
      ...
      if( azimuth<0 )azimuth=azimuth+360.0D0
